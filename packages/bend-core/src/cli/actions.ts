...
export async function createProject(a: Answers) {
  ...
  if (pm !== a.pkgm) console.warn(pc.yellow(`${a.pkgm} not found, falling back to npm.`));
  ...
  await copyDirRenderEJS(path.join(TEMPLATES, "server", a.framework, langDir), projectDir, ctx);

  // orm
  if (a.orm === "mongoose") {
    await copyDirRenderEJS(path.join(TEMPLATES, "orm", "mongoose", langDir), projectDir, ctx);
  } else if (a.orm === "prisma") {
    // Always schema
    await copyDirRenderEJS(path.join(TEMPLATES, "orm", "prisma", "prisma"), projectDir, ctx);
    // TS helper only for TypeScript projects
    if (a.language === "typescript") {
      await copyDirRenderEJS(path.join(TEMPLATES, "orm", "prisma", "ts"), projectDir, ctx);
    }
  }
